...
var config = {
    ...
    colors: ['#72CAA0', '#f7a35c', '#434348', '#90ed7d', '#f15c80', '#8085e9'],
...
function calValidRightDistance(chartData, config, opts) {

    var dataChartAreaWidth = opts.width - config.padding - chartData.xAxisPoints[0];
    var dataChartWidth = chartData.eachSpacing * opts.categories.length;
    var validDistance = dataChartAreaWidth - dataChartWidth;
    return validDistance;
}
...
function calLegendData(series, opts, config) {
    ...
    var currentRow = [];
    series.forEach(function (item) {
        var itemWidth = 1 * padding + shapeWidth + measureText(item.name || 'undefined');
...
function drawXAxis(categories, opts, config, context) {
    ...
    if (config._xAxisTextAngle_ === 0) {
        ...
        categories.forEach(function (item, index) {
            ...
            context.fillText(item, xAxisPoints[index] + offset, startY + 8 + 5);
            context.closePath();
            context.stroke();
            context.restore();
        });
...
function drawYAxis(series, opts, config, context) {
    ...
    rangesFormat.forEach(function (item, index) {
        ...
        context.fillText(item, config.padding + config.yAxisTitleWidth, pos + 14 / 2);
    });
    context.closePath();
    context.stroke();
...
Charts.prototype.scrollToEnd = function () {
    if (this.opts.enableScroll === true) {

        var validDistance = calValidRightDistance(this.chartData, this.config, this.opts);

        this.scrollOption.currentOffset = validDistance
        var opts = assign({}, this.opts, {
